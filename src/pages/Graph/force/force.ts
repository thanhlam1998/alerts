 ...
 */
export class TestForceLayout extends Base {
  /** 向心力作用点 */
  ...
  public init(data: Model) {
    ...
    self.edges = edges.map((edge: any) => {
      const res: any = {};
      const expectKeys = ["targetNode", "sourceNode", "startPoint", "endPoint"];
      Object.keys(edge).forEach((key: any) => {
  ...
  public execute(reloadData?: boolean) {
    ...
    if (!simulation) {
      try {
        ...
        // console.log('worker enabled: true');
          // worker is enabled
          simulation.stop();
        ...
        //   console.log('worker enabled: false');
        //   simulation
        //     .on("tick", () => {
        //       self.tick();
        //     })
